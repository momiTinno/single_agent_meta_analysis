...
describe("run validation middleware", () => {
  it("attaches valid input for the run controller", () => {
    ...
    let nextError;
    validateRunInput(request, {}, (error) => { nextError = error; });
    expect(nextError).toBeUndefined(); expect(request.validatedInput.bmc.h0).toBe("Test");
  });
  it("passes malformed input to the error middleware", () => {
    let nextError;
    validateRunInput({ body: { turns: [] } }, {}, (error) => { nextError = error; });
