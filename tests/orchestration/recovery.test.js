...
describe("recovery semantics", () => {
  it("resumes a pending action and uses its original call ID", async () => {
    const run = {
      ...
      messages: [],
      ctx: {
        phase1: { callId: "saved", attempt: 1, output: { findings: [] } },
      ...
      pendingAction: { callId: "saved", name: "run_phase_1", args: { retryHint: null } },
      step: 0,
    };
    let saved;
    const store = {
    ...
    expect(response.id).toBe("saved");
    expect(response.response.reused).toBe(true);
  });
  it("adopts each stale running row on boot", async () => {
    const adopted = [];
    const store = { staleRunning: async () => [{ runId: "one" }, { runId: "two" }] };
