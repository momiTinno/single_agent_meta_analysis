...
export async function runAgent(runId, deps = {}) {
  ...
  while (true) {
    let run = await store.load(runId); if (!run || run.status !== "running") return run?.status;
    if (run.step >= config.MAX_STEPS) return finish(store, run, { code: "MAX_STEPS", message: "Maximum orchestration steps reached" }, log, startedAt);
    let action = run.pendingAction;
    if (!action) {
      log.info({ event: "agent_request_started", step: run.step, messageCount: run.messages.length, attempts: run.attempts }, "agent_request_started");
      ...
      if (calls.length !== 1) return finish(store, run, { code: "INVALID_TOOL_RESPONSE", message: "Expected exactly one function_call" }, log, startedAt);
      const call = calls[0]; if (!known.has(call.name)) return finish(store, run, { code: "UNKNOWN_TOOL", message: call.name }, log, startedAt);
      if (call.args !== undefined && (call.args === null || typeof call.args !== "object" || Array.isArray(call.args))) return finish(store, run, { code: "INVALID_TOOL_ARGS", message: call.name }, log, startedAt);
      ...
      log.info({ event: "agent_action_selected", step: run.step, tool: action.name, callId: action.callId, phase, attempt: phase ? attempts[phase] : undefined, hasRetryHint: Boolean(action.args.retryHint) }, "agent_action_selected");
      await store.savePending(runId, { messages: [...run.messages, content], pendingAction: action, attempts });
      log.info({ event: "pending_action_saved", step: run.step, tool: action.name, callId: action.callId }, "pending_action_saved"); run = await store.load(runId);
    }
    ...
    log.info({ event: "tool_result_saved", step: run.step, tool: action.name, callId: action.callId, durationMs, status: result.status || "running" }, "tool_result_saved");
    if (result.status && result.status !== "running") {
      const terminalLog = result.status === "success" ? atLevel(log, "info") : atLevel(log, "warn");
      terminalLog({ event: "run_terminal", status: result.status, step: run.step + 1, durationMs: Date.now() - startedAt, error: result.error?.code || undefined }, "run_terminal");
...
async function execute(run, action, phaseRunners, callGemini, log) {
  ...
  if (phase) {
    if (run.attempts[phase] > config.MAX_ATTEMPTS_PER_PHASE) {
      atLevel(log, "warn")({ event: "phase_attempt_limit_reached", phase, attempt: run.attempts[phase], maxAttempts: config.MAX_ATTEMPTS_PER_PHASE }, "phase_attempt_limit_reached");
    ...
    log.info({ event: "phase_execution_started", phase, attempt: run.attempts[phase], turnCount: run.input.turns.length, hypothesisCount: Object.keys(run.input.bmc).length, hasRetryHint: Boolean(action.args.retryHint) }, "phase_execution_started");
  ...
  if (action.name.startsWith("validate_phase_")) {
    ...
    validationLog({ event: validation.ok ? "phase_validation_passed" : "phase_validation_failed", phase: key, issueCount: validation.issues.length, issues: validation.ok ? undefined : validation.issues }, validation.ok ? "phase_validation_passed" : "phase_validation_failed");
    return { ctx, payload: validation };
  }
  if (action.name === "finalize") {
    const valid = ["phase1", "phase2", "phase3"].every((key) => ctx[`${key}Validation`]?.ok);
    if (!valid) {
      atLevel(log, "warn")({ event: "finalization_blocked", validationStates: Object.fromEntries(["phase1", "phase2", "phase3"].map((key) => [key, ctx[`${key}Validation`]?.ok || false])) }, "finalization_blocked");
      return { payload: { ok: false, issues: ["cannot finalize before all validations pass"] } };
    }
    const artifact = finalize(run); log.info({ event: "artifact_finalized", themeCount: artifact.thematicAnalysis.themes.length, metaInsightCount: artifact.metaInsights.length, analysisCount: artifact.analysis.length }, "artifact_finalized"); return { status: "success", artifact, ctx, payload: { ok: true, summary: "run finalized" } };
  }
  if (action.name === "abort_non_retryable") { const error = abort(run, action.args.reason); atLevel(log, "warn")({ event: "run_abort_requested", reason: error.reason || error.message || undefined }, "run_abort_requested"); return { status: "aborted", error, ctx, payload: { ok: true, summary: "run aborted" } }; }
  return { status: "failed", error: { code: "UNKNOWN_TOOL", message: action.name }, ctx, payload: { ok: false } };
}

async function finish(store, run, error, log, startedAt) { await store.fail(run.runId, error); log.error({ event: "run_failed", error: error.code, durationMs: Date.now() - startedAt }, "run_failed"); return "failed"; }
